...
public sealed class GameDataPackageLoaderTests
{
    ...
    [Fact]
    public async Task LoadFromFileAsync_ActiveUniqueCompositionArtifact_RemainsCompatible()
    {
        ...
        Assert.True(result.IsSuccess);
        Assert.NotNull(result.Package);
        Assert.Equal("3.29.1.2.2-deprecated-source-mechanics-resolution", result.Package.Manifest.DataVersion);
