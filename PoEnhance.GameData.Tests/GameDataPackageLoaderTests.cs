...
public sealed class GameDataPackageLoaderTests
{
    ...
    [Fact]
    public async Task LoadFromFileAsync_ActiveUniqueOptionAxisArtifact_RemainsCompatible()
    {
        ...
        Assert.True(result.IsSuccess);
        Assert.NotNull(result.Package);
        Assert.Equal("3.29.1.2.2-unique-option-axes", result.Package.Manifest.DataVersion);
