...
public sealed class GameDataPackageLoaderTests
{
    ...
    [Fact]
    public async Task LoadFromFileAsync_ActiveUniqueSemanticFingerprintArtifact_RemainsCompatible()
    {
        ...
        Assert.True(result.IsSuccess);
        Assert.NotNull(result.Package);
        Assert.Equal("3.29.1.2.2-unique-semantic-fingerprint", result.Package.Manifest.DataVersion);
