...
- Parsed items can be enriched with item-base resolution and Advanced metadata modifier candidate discovery in `PoEnhance.Core` when a caller supplies a validated `GameDataCatalog`; modifier candidates can be narrowed by verified base eligibility and dynamic traditional-influence item-context tags only when exactly one catalog item base is resolved. Unresolved or ambiguous bases preserve earlier candidates. Tier inference, modifier value-range matching, and fuzzy matching are not implemented yet.
