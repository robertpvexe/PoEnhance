...
| DEC-033 | Provisional | Definicje statystyk i tłumaczenia statystyk RePoE są importowane wyłącznie z lokalnych plików do provider-neutral rekordów GameData. Identyfikatory tłumaczeń są deterministycznymi hashami uporządkowanej krotki source stat-id, gdy RePoE nie dostarcza stabilnego id rekordu. | Współdzielone katalogi potrzebują metadanych statystyk i reguł tekstu przedmiotów bez przenoszenia kształtów RePoE do modeli runtime. | `PoEnhance.GameData` może walidować aliasy statystyk, referencje statystyk modifierów, referencje tłumaczeń, warunki, index handlers i format lines, a `PoEnhance.DataImport` posiada parsowanie JSON specyficzne dla RePoE. | Rozważyć ponownie przy dodawaniu kolejnych języków, bezpośredniego importera RePoE `mods.json`, mapowania Trade stat-id lub aktywacji pakietów runtime. |
